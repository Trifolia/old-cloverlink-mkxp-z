...
static void mriBindingInit() {
  ...
  _rb_define_module_function(mod, "is_really_windows?",
                             mkxpIsReallyWindowsHost);
  ...
  _rb_define_module_function(mod,
                             "default_font_family=", mkxpSetDefaultFontFamily);
  ...
  std::string version =
      std::string(MKXPZ_VERSION "/") + getPlistValue("GIT_COMMIT_HASH");
  ...
  int state;
  rb_eval_string_protect("require('zlib') if !Kernel.const_defined?(:Zlib)",
                         &state);
  if (state) {
    Debug() << "Could not load Zlib. If this is important, make sure Ruby was "
               "built with static extensions, or that"
            << ((MKXPZ_PLATFORM == MKXPZ_PLATFORM_MACOS) ? "zlib.bundle"
                                                         : "zlib.so")
...
RB_METHOD(mkxpIsReallyMacHost) {
  RB_UNUSED_PARAM;
  return rb_bool_new(mkxp_sys::getRealHostType() ==
                     mkxp_sys::WineHostType::Mac);
}

RB_METHOD(mkxpIsReallyLinuxHost) {
  RB_UNUSED_PARAM;
  return rb_bool_new(mkxp_sys::getRealHostType() ==
                     mkxp_sys::WineHostType::Linux);
}

RB_METHOD(mkxpIsReallyWindowsHost) {
  RB_UNUSED_PARAM;
  return rb_bool_new(mkxp_sys::getRealHostType() ==
                     mkxp_sys::WineHostType::Windows);
}
...
#else
    Debug() << command << ":"
            << "Arguments are not supported with xdg-open. Ignoring.";
#endif
  }
...
json5pp::value loadUserSettings() {
  ...
  if (rb_funcall(rb_cFile, rb_intern("exists?"), 1, cpath) == Qtrue) {
    VALUE f =
        rb_funcall(rb_cFile, rb_intern("open"), 2, cpath, rb_str_new("r", 1));
...
void saveUserSettings(json5pp::value &settings) {
  VALUE cpath = rb_utf8_str_new_cstr(shState->config().userConfPath.c_str());
  VALUE f =
      rb_funcall(rb_cFile, rb_intern("open"), 2, cpath, rb_str_new("w", 1));
  rb_funcall(f, rb_intern("write"), 1,
             rb_utf8_str_new_cstr(
                 settings.stringify5(json5pp::rule::space_indent<>()).c_str()));
  rb_funcall(f, rb_intern("close"), 0);
}
...
#endif
#ifdef MKXPZ_NO_OPENAL
  const char *str = "Graphics.__reset__;";
#endif
...
RB_METHOD(_kernelCaller) {
  ...
  VALUE args[] = {rb_utf8_str_new_cstr(":in `<main>'"),
                  rb_utf8_str_new_cstr("")};
...
static void runRMXPScripts(BacktraceData &btData) {
  ...
  if (scriptPack.empty()) {
    showMsg("No script file has been specified. Check the game's INI and try "
            "again.");
    return;
  }
...
static void mriBindingExecute() {
  ...
  if (state || !valid) {
     ...
     */
    showMsg(
        "An error occurred while initializing Ruby. (Invalid JIT settings?)");
    ruby_cleanup(state);
  ...
  resPath += "/Ruby/" + std::to_string(RAPI_MAJOR) + "." +
             std::to_string(RAPI_MINOR) + ".0";
  ...
  //     else {
  //         rb_ary_push(lpaths,
  //         rb_utf8_str_new_cstr(mkxp_fs::getCurrentDirectory().c_str()));
  //     }
  // #endif
