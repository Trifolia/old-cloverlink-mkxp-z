...
std::string sourceDescString(const SourceDesc &src) {
  ...
  case CAxis:
    snprintf(buf, sizeof(buf), "%s%c",
             shState->input().getAxisName(src.d.ca.axis),
             src.d.ca.dir == Negative ? '-' : '+');
    return buf;
  }
...
struct BindingWidget : Widget {
  ...
  BindingWidget(int vbIndex, SMP *p, const IntRect &rect)
      : Widget(p, rect), vb(vButtons[vbIndex]), hoveredCell(-1) {}
...
struct Label : Widget {
  ...
  Label(SMP *p, const IntRect &rect, const char *str, uint8_t r, uint8_t g,
        uint8_t b)
      : Widget(p, rect), str(str), visible(true) {
    c.r = r;
    c.g = g;
    c.b = b;
    c.a = 255;
...
enum State { Idle, AwaitingInput };

enum Justification { Left, Center };
...
struct SettingsMenuPrivate {
  ...
  SettingsMenuPrivate(RGSSThreadData &rtData) : rtData(rtData) {}

  SDL_Surface *createSurface(int w, int h) {
    int bpp;
    Uint32 rMask, gMask, bMask, aMask;
    SDL_GetMasksForPixelFormat(SDL_PIXELFORMAT_ABGR8888, &bpp, &rMask, &gMask,
                               &bMask, &aMask);

    return SDL_CreateSurface(
        w, h, SDL_GetPixelFormatForMasks(bpp, rMask, gMask, bMask, 0));
  }

  void fillSurface(SDL_Surface *surf, uint8_t grey) {
    SDL_FillSurfaceRect(surf, 0,
                        SDL_MapSurfaceRGBA(surf, grey, grey, grey, 255));
  }

  void fillRect(SDL_Surface *surf, int x, int y, int w, int h, uint8_t r,
                uint8_t g, uint8_t b) {
  ...
  void strokeLineH(SDL_Surface *surf, uint8_t grey, int x, int y, int length,
                   int width) {
    fillRect(surf, grey, x, y - width / 2, length, width);
  }

  void strokeLineV(SDL_Surface *surf, uint8_t grey, int x, int y, int length,
                   int width) {
    fillRect(surf, grey, x - width / 2, y, width, length);
  }

  void strokeLineH(SDL_Surface *surf, uint8_t r, uint8_t g, uint8_t b, int x,
                   int y, int length, int width) {
    fillRect(surf, r, g, b, x, y - width / 2, length, width);
  }

  void strokeLineV(SDL_Surface *surf, uint8_t r, uint8_t g, uint8_t b, int x,
                   int y, int length, int width) {
    fillRect(surf, r, g, b, x - width / 2, y, width, length);
  }

  void strokeRect(SDL_Surface *surf, uint8_t grey, int x, int y, int w, int h,
                  int lineW) {
  ...
  void strokeRectInner(SDL_Surface *surf, int x, int y, int w, int h, int lineW,
  ...
  void strokeRectInner(SDL_Surface *surf, uint8_t grey, int x, int y, int w,
                       int h, int lineW) {
  ...
  SDL_Surface *createTextSurface(const char *str, bool bold) {
    ...
    return TTF_RenderText_Blended(font, str, 0, c);
  }
  ...
  void blitTextSurf(SDL_Surface *surf, int x, int y, int alignW,
                    SDL_Surface *txtSurf, Justification just) {
    SDL_Rect dstRect;
    dstRect.x = drawOff.x;
  ...
  void drawText(SDL_Surface *surf, const char *str, int x, int y, int alignW,
  ...
  void redraw() {
    ...
    if (state == AwaitingInput) {
      char buf[64];
      snprintf(buf, sizeof(buf), "Press key or joystick button for \"%s\"",
               captureName);
  ...
  void onCancel() { destroyReq = true; }
};

Widget::Widget(SMP *p, const IntRect &rect) : p(p), rect(rect) {}

bool Widget::hit(int x, int y) { return pointInRect(rect, x, y); }
...
void Widget::motion(int x, int y) { motionHandler(x - rect.x, y - rect.y); }
...
void BindingWidget::drawHandler(SDL_Surface *surf) {
  ...
  const int cellOff[] = {cellOffX, 1,     cellOffX + cellW, 1,
                         cellOffX, cellH, cellOffX + cellW, cellH};

  const int lbOff[] = {0, cellH / 2,         cellW, cellH / 2,
                       0, cellH + cellH / 2, cellW, cellH + cellH / 2};

  /* Hovered cell background */
  if (hoveredCell != -1)
    p->fillRect(surf, cBgDark, cellOff[hoveredCell * 2],
                cellOff[hoveredCell * 2 + 1], cellW, cellH);
...
void BindingWidget::leaveHandler() { setHoveredCell(-1); }
...
SettingsMenu::SettingsMenu(RGSSThreadData &rtData) {
  ...
  p->window = SDL_CreateWindow("Key bindings", winSize.x, winSize.y,
  ...
  for (int y = 0; y < (int)layoutH; ++y)
    for (int x = 0; x < (int)layoutW; ++x) {
      ...
      BindingWidget w(
          i, p,
          IntRect(x * bWidgetW, bWidgetY + y * bWidgetH, bWidgetW, bWidgetH));
  ...
  p->buttons.push_back(
      Button(p, btRects[0], "Reset defaults", &SMP::onResetToDefault));
  ...
  /* Labels */
  const char *info =
      "Use left click to bind a slot, right click to clear its binding";
  p->infoLabel =
      Label(p, IntRect(16, 16, winSize.x - 32, 16), info, cText, cText, cText);

  const char *warn = "Warning: Same physical action bound to multiple slots";
  p->dupWarnLabel =
      Label(p, IntRect(16, 40, winSize.x - 32, 16), warn, 255, 0, 0);
...
bool SettingsMenu::onEvent(const SDL_Event &event) {
  ...
  case SDL_EVENT_WINDOW_SHOWN: // SDL is bugged and doesn't give us a first
                               // FOCUS_GAINED event
...
void SettingsMenu::raise() { SDL_RaiseWindow(p->window); }

bool SettingsMenu::destroyReq() const { return p->destroyReq; }
